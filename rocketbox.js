...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Observer {
        ...
        draw() {
            ...
            for(let t = 1;t<this.points.length;t++){
                canvas_context.lineTo(this.points[t][0], this.points[t][1])
                // canvas_context.lineTo(this.body.x, this.body.y)
            }
    ...
    class Rocketbox {
        constructor(x,y){
            ...
            this.angle = 0
            this.observer = new Observer(this.body.x+(this.body.width*.5), this.body.y+(this.body.height*.5), 5, "red", 1000, 1, 0)
        }
        draw(){
            // this.body.draw()
            this.observer.draw()
    ...
    class Player{
        constructor(){
            this.body = new Circle(500,100, 5, "#FFAA00")
        }
        draw(){
            control(this, 5)
            this.body.draw()
    ...
    let blocks = []
    for(let t = 0; blocks.length<100; t++){
        let wet = 0
        let block = new Rectangle(Math.random()*700, Math.random()*700, 5, 5, "white")
        ...
        for(let k = 0;k<blocks.length;k++){
            let link = new LineOP(block, blocks[k])
    ...
    let block = new Rectangle(450, 450, 100, 50, "white")
    let player = new Player()
    let rocker = new Rocketbox(325,325)
    rocker.observer.obstacles.push(...blocks)
    function main() {
        ...
        rocker.observer.globalangle = ((rocker.observer.globalangle*99)+(Math.PI*1)+(Math.atan2(rocker.observer.body.y-player.body.y,rocker.observer.body.x-player.body.x)))/100
        }
